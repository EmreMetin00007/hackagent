...
# Profil → server listesi
case "$PROFILE" in
  min)   SERVERS="kali-tools memory-server telemetry" ;;
  recon) SERVERS="kali-tools osint-tools browser memory-server telemetry" ;;
  web)   SERVERS="kali-tools web-advanced llm-security browser memory-server rag-engine telemetry" ;;
  llm)   SERVERS="llm-security browser web-advanced memory-server telemetry" ;;
  ctf)   SERVERS="kali-tools ctf-platform memory-server rag-engine telemetry" ;;
  ad)    SERVERS="kali-tools ad-tools container-tools memory-server telemetry" ;;
  full)  SERVERS="kali-tools web-advanced ctf-platform ad-tools memory-server container-tools osint-tools telemetry browser rag-engine llm-security" ;;
...
CCO_DIR="$CCO_DIR" CCO_DATA_DIR="$CCO_DATA_DIR" SERVERS="$SERVERS" PROFILE="$PROFILE" \
python3 <<'PYEOF'
import json, os
...
for name in servers:
    ...
    env = env_for(name)
    # Kullanıcı daha önce token girdiyse koru
